import { BotSchema } from './db-schemas/bot'
import { getDb } from './db'
import { requestBot } from './send'
...
fastify.post<{ Body: { sessionID: string; input: string; output: string } }>(
  '/assert',
  {
    schema: {
      body: {
    ...
    handler: async (request, reply) => {
      try {
        const outputs = await requestBot(
